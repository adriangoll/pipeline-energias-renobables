    ...
    try:
        ...
        for obj in response['Contents']:
            if obj['Key'].endswith('.parquet'):
                ...
                # Extraer columnas de partición desde el path
                parts = obj['Key'].split('/')
                for part in parts:
                    if '=' in part:
        ...
        for hour in range(24):
            for city in ['Riohacha', 'Patagonia']:
                ...
                data.append({
                    ...
                    'velocidad_viento_promedio': wind,
                    'temperatura_promedio': temp,
                    'nubosidad_promedio': clouds,
...
if use_real_data:
    ...
    df_solar = df_sample[['city', 'year', 'month', 'day', 'hour',
                           'potencial_solar_promedio', 'temperatura_promedio', 'nubosidad_promedio']].copy()
    df_eolico = df_sample[['city', 'year', 'month', 'day', 'velocidad_viento_promedio_ms']].copy()
    df_ranking = df_sample[['city', 'date', 'score_total',
                             'potencial_solar_promedio', 'velocidad_viento_promedio']].copy()
    df_comparativo = df_sample.groupby(['city', 'year', 'month']).agg({
        'potencial_solar_promedio': 'mean',
        'velocidad_viento_promedio': 'mean',
...
with col1:
    st.metric("Total de Días Analizados", f"{len(df_ranking):,}")

with col2:
    avg_solar = df_solar['potencial_solar_promedio'].mean()
    st.metric("Potencial Solar Promedio", f"{avg_solar:.2f} kWh/m²")

with col3:
    avg_wind = df_eolico['velocidad_viento_promedio'].mean()
...
df_horaria = df_p1.groupby('hour')['potencial_solar_promedio'].mean().reset_index()
...
df_mensual = df_solar[df_solar['city'] == ciudad_filtro].groupby('month')['potencial_solar_promedio'].mean().reset_index()
...
fig2 = px.bar(
    ...
    color='potencial_solar_promedio',
...
df_eolico_comp = df_eolico.groupby(['city', 'month'])['velocidad_viento_promedio_ms'].mean().reset_index()
...
fig3 = px.line(
    ...
    labels={'month': 'Mes', 'velocidad_viento_promedio_ms': 'Velocidad Viento (m/s)', 'city': 'Ciudad'},
    markers=True
)
...
fig5 = px.scatter(
    ...
    labels={'nubosidad_promedio': 'Nubosidad (%)',
...
with col1:
    df_comp_solar = df_comparativo.groupby('city')['potencial_solar_promedio'].mean().reset_index()
    fig6 = px.pie(
        df_comp_solar,
        values='potencial_solar_promedio',
        names='city',
...
with col2:
    df_comp_wind = df_comparativo.groupby('city')['velocidad_viento_promedio'].mean().reset_index()
    fig7 = px.bar(
        df_comp_wind,
        x='city',
        y='velocidad_viento_promedio',
        title='Velocidad Viento Promedio por Ciudad',
        labels={'city': 'Ciudad', 'velocidad_viento_promedio': 'Viento (m/s)'},
        color='city',
        color_discrete_sequence=['#FFA500', '#1E90FF']
    )
...
comp_table = df_comparativo.groupby('city').agg({
    'potencial_solar_promedio': ['mean', 'max'],
    'velocidad_viento_promedio': ['mean', 'max'],
    'temperatura_promedio': 'mean'
}).round(2)
...
st.dataframe(
    df_top10[['date', 'score_total', 'potencial_solar_promedio', 'velocidad_viento_promedio_ms']].reset_index(drop=True),
